...
class City:
    ...
    def adjust_traffic_weight(self, road_start, road_end, new_weight):
        ...
        if road_end in self.graph[road_start]:
            ...
            print(
                f"Traffic weight between intersections {road_start} and {road_end} updated from {old_weight} to {new_weight}")
            return True
        else:
            print(f"ERROR: No road exists between intersections {road_start} and {road_end}")
            return False
    ...
    def visualize(self, path1=None, path2=None, start=None, end=None, path1_color='red', path2_color='blue',
                  path1_label=None, path2_label=None, title=" Doyrampur Mega City (DMC) Inside AT BAUET  "):
...
def user_input_mode():
    ...
    while True:
        ...
        print("13. Determine optimal path")
        print("14. Exit")
        ...
        if choice == '1':
            ...
            print(Fore.GREEN + Style.BRIGHT + "Goodbye, and drive safely! And Assalamu Alaikum to the Drivers!")
            break
